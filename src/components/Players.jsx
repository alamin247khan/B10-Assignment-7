...
const FLAG = 'https://upload.wikimedia.org/wikipedia/en/4/41/Flag_of_India.svg';
...
export default function Players({ coin, setCoin, selectedPlayers, setSelectedPlayers }) {
  ...
  const playersToShow = showSelected
    ? selectedPlayers
    : PLACEHOLDER_PLAYERS.slice(0, visibleCount);
  ...
  return (
    <div className="w-full px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex">
          <button
            className={`px-6 py-2 font-semibold text-[16px] border border-[#1313131A] rounded-l-[12px] ${!showSelected ? 'bg-[#e7fe29]' : 'bg-white'}`}
            onClick={handleShowAvailable}
          >
            Available
          </button>
          <button
            className={`px-6 py-2 font-semibold text-[16px] border-t border-b border-r border-[#1313131A] rounded-r-[12px] ${showSelected ? 'bg-[#e7fe29]' : 'bg-white'}`}
          ...
          >
            ...
            <div className="w-full flex flex-col items-center px-4 py-3">
              ...
              <div className="flex items-center gap-2 mt-2 mb-1">
                <svg width="24" height="24" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="8" r="4" fill="#e7fe29" /><rect x="6" y="14" width="12" height="6" rx="3" fill="#e7fe29" /></svg>
              ...
              <div className="flex items-center justify-between w-full mb-1">
                <span className="text-xs font-semibold text-[#131313] bg-[#e7fe29] px-3 py-1 rounded-full">{player.role}</span>
                <button className="ml-auto px-2 py-1 text-xs rounded bg-gray-100 font-semibold">Info</button>
              </div>
              {/* Rating and stars */}
              <div className="flex items-center justify-between w-full mb-1">
                ...
                <span className="flex items-center ml-2">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} width="16" height="16" fill={i < player.rating ? '#FFD700' : '#E0E0E0'} viewBox="0 0 24 24"><polygon points="12,2 15,9 22,9 17,14 18,21 12,17 6,21 7,14 2,9 9,9" /></svg>
                  ))}
                </span>
              </div>
              {/* Price and Choose/Remove Player */}
              <div className="flex items-center justify-between w-full mb-2 mt-2">
                <span className="font-bold text-[#b8860b]">{player.price.toLocaleString()} coin</span>
                {!showSelected && (
                  <button
                    className="ml-2 px-4 py-1.5 rounded-lg font-semibold text-black bg-[#e7fe29] border border-[#1313131A] disabled:opacity-50"
                    disabled={coin < player.price || selectedPlayers.some(p => p.id === player.id)}
